...
export default function PatientMeetings() {
	...
	const openChat = (request: MeetingRequest) => {
		if (!request.expertId) {
			alert(
				"Cannot chat with this researcher - researcher not on platform"
			);
			return;
		}
	...
	const renderRequest = (request: MeetingRequest) => (
		<Card key={request._id}>
			<CardHeader>
				<div className="flex items-start justify-between">
					<div className="flex-1">
						...
						{request.expertId?.specialties &&
							request.expertId.specialties.length > 0 && (
			...
			<CardContent className="space-y-4">
				...
				<div>
					<p className="text-sm font-medium mb-2 text-gray-700 dark:text-gray-500">
						Your Request Message:
					</p>
				...
				{request.status === "pending" && (
					<div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border-l-4 border-blue-400">
						<p className="text-sm text-blue-700 flex items-center gap-2">
							...
							Your meeting request is pending. The researcher will
							review it soon.
						</p>
					</div>
				)}
				...
				{request.status === "accepted" && !request.responseMessage && (
					<div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border-l-4 border-green-500">
						<p className="text-sm text-green-700 dark:text-green-300 flex items-center gap-2">
							...
							Your meeting request has been accepted! Start a
							conversation below.
						</p>
					</div>
				)}
		...
		<div className="max-w-6xl mx-auto">
			<div className="mb-8">
				...
				<p className="text-gray-600">
					Track your meeting requests with researchers and start
					conversations
				</p>
			</div>
						...
						<Card>
							<CardContent className="pt-6 text-center py-8">
								...
								<p className="text-sm text-muted-foreground">
									Find researchers in the Health Experts
									section and request meetings
								</p>
							</CardContent>
						</Card>
					) : (
					...
					{acceptedRequests.length === 0 ? (
						<Card>
							<CardContent className="pt-6 text-center py-8">
								...
								<p className="text-muted-foreground">
									No accepted meeting requests yet
								</p>
							</CardContent>
						</Card>
					) : (
			...
			<Dialog open={chatDialogOpen} onOpenChange={setChatDialogOpen}>
				<DialogContent className="max-w-lg">
					<DialogHeader>
						<DialogTitle>
							Start Chat with Dr. {chatExpert?.name}
						</DialogTitle>
						<DialogDescription>
