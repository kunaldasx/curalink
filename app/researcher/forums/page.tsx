...
export default function ResearcherForums() {
	...
	const fetchTopics = async (categoryId: string) => {
		...
		try {
			const res = await fetch(
				`/api/forum/topics?categoryId=${categoryId}&sortBy=${sortBy}`
			);
			const data = await res.json();
	...
	const handleAskQuestion = () => {
		if (selectedCategory) {
			router.push(
				`/researcher/forums/new?category=${selectedCategory._id}`
			);
		}
	};
					...
					<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
						<div>
							...
							<p className="text-lg text-gray-700">
								💬 Create communities, answer patient questions,
								and share medical expertise
							</p>
						</div>
						<Button
						...
						<div className="flex items-start gap-4">
							...
							<div className="flex-1">
								...
								<div className="space-y-2 text-sm text-gray-700">
									...
									<p className="flex items-center gap-2">
										✓ Answer patient questions with verified
										responses
									</p>
									<p className="flex items-center gap-2">
										✓ Lead discussions and share expertise
									</p>
									<p className="flex items-center gap-2">
										✓ Pin, hide, and flag content for
										quality control
									</p>
								</div>
							</div>
				...
				>
					<DialogContent className="sm:max-w-[600px]">
						<DialogHeader>
							...
							<DialogDescription>
								Create a new discussion space for patients and
								researchers
							</DialogDescription>
						</DialogHeader>
		...
		<div className="max-w-7xl mx-auto">
			...
			<div className="space-y-3">
				...
				{topics.map((topic) => (
					<Link
						key={topic._id}
						href={`/researcher/forums/topic/${topic._id}`}
					>
