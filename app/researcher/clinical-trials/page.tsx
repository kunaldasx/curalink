...
export default function ResearcherTrials() {
	const [activeTab, setActiveTab] = useState("my-trials");

	// My Trials state
	const [myTrials, setMyTrials] = useState<any[]>([]);
	const [loadingMyTrials, setLoadingMyTrials] = useState(false);

	// Browse All Trials state
	...
	const fetchMyTrials = async () => {
		setLoadingMyTrials(true);
		try {
			const res = await fetch("/api/researcher/trials");
			const data = await res.json();
			setMyTrials(data.trials || []);
		} catch (error) {
			console.error("Fetch error:", error);
		} finally {
			setLoadingMyTrials(false);
		}
	};
	...
	const openEmailDialog = (trial: any) => {
		...
		setEmailBody(
			`Dear Trial Administrator,\n\n` +
				...
				`I believe my expertise could complement this research.\n\n` +
				`Best regards`
		);
		...
		<div className="max-w-7xl mx-auto">
			...
			<div className="mb-8 p-6 md:p-8 rounded-3xl bg-gradient-to-br from-medical-teal-50 via-medical-indigo-50 to-medical-lavender-50 border border-medical-teal-100 shadow-lg animate-fade-in-up">
				...
				<p className="text-lg text-gray-700">
					🧬 Manage your trials and discover collaboration
					opportunities
				</p>
			</div>
				...
				<TabsContent value="my-trials" className="space-y-4">
					<div className="flex justify-between items-center mb-4">
						<p className="text-sm text-muted-foreground">
							Manage your clinical trials and research studies
						</p>
						...
						<Card>
							<CardContent className="pt-6 text-center py-8">
								...
								<p className="text-muted-foreground mb-4">
									You haven't added any trials yet.
								</p>
								<Link href="/researcher/clinical-trials/new">
									<Button>
										<Plus className="mr-2 h-4 w-4" />
								...
								<Card key={i}>
									<CardHeader>
										<div className="flex justify-between items-start gap-4">
											...
											<Link
												href={`/researcher/clinical-trials/manage/${trial._id}`}
											>
												<Button
													variant="outline"
													size="sm"
				...
				<TabsContent value="browse" className="space-y-4">
							...
							<div className="space-y-4">
								...
								<div>
									...
									<p className="text-xs text-muted-foreground mt-1">
										...
										treatment type, phase, or keywords
									</p>
								</div>
					...
					{allTrials.length > 0 && (
						<div className="flex items-center justify-between">
							<p className="text-sm text-muted-foreground">
								Showing {allTrials.length} trial
								{allTrials.length !== 1 ? "s" : ""}
							</p>
						</div>
					)}
					...
					{/* Results */}
					<div className="space-y-4">
						{allTrials.length === 0 && !loading && (
							<Card>
								<CardContent className="pt-6 text-center py-8">
									<Search className="h-12 w-12 text-gray-300 mx-auto mb-3" />
									<p className="text-muted-foreground mb-2">
										No trials found. Try different search
										terms.
									</p>
								</CardContent>
							</Card>
				...
				<DialogContent className="max-w-2xl">
					...
					<div className="space-y-4">
						...
						<div>
							...
							/>
							<p className="text-xs text-muted-foreground mt-1">
								Customize this message to introduce yourself and
					...
					<DialogFooter>
						...
						<Button
							onClick={sendEmail}
							disabled={
								sendingEmail || !emailSubject || !emailBody
							}
						>
							{sendingEmail ? (
								<>
