...
export default function ResearcherCollaborators() {
	...
	const [savedCollaborators, setSavedCollaborators] = useState<Set<string>>(
		new Set()
	);
	...
	const handleSearch = async () => {
		...
		try {
			const res = await fetch(
				`/api/collaborators/search?query=${encodeURIComponent(query)}`
			);
			...
			if (response.ok) {
				if (data.isFavorite) {
					setSavedCollaborators((prev) =>
						new Set(prev).add(collaboratorId)
					);
				} else {
					setSavedCollaborators((prev) => {
	...
	const sendConnectionRequest = async (recipientId: string) => {
		...
		try {
			...
			console.log(
				"Connection request sent successfully, actionLoading cleared"
			);
		} catch (error) {
	...
	const removeConnection = async (connectionId: string) => {
		...
		try {
			const response = await fetch(
				`/api/collaborators/connections?connectionId=${connectionId}`,
				{
					method: "DELETE",
				}
			);
			...
			<CardHeader>
				<div className="flex items-start justify-between">
					...
					<div className="flex gap-2 items-start">
						<Button
							variant={
								savedCollaborators.has(collab._id)
									? "default"
									: "ghost"
							}
							size="icon"
							onClick={() =>
								handleToggleSave(collab._id, collab.name)
							}
							title={
								savedCollaborators.has(collab._id)
									? "Remove from library"
	...
	const renderConnectionButton = (collab: any) => {
		...
		const isLoading =
			...
			(actionLoading === collab._id ||
				actionLoading === collab.connectionId);
						...
						<Card>
							<CardContent className="pt-6 text-center py-8">
								...
								<p className="text-muted-foreground">
									Search for researchers by specialty or
									research interest
								</p>
							</CardContent>
						</Card>
					...
					{connections.accepted.length === 0 && (
						<Card className="col-span-2">
							<CardContent className="pt-6 text-center py-8">
								...
								<p className="text-muted-foreground">
									No connections yet. Search for collaborators
									to get started!
								</p>
							</CardContent>
						</Card>
